...
def generate_image_paths_and_labels(network_type, input_dir, split, read_from_folders, file_name, classes):
    #TODO: Change segmentation to localisation and implement actaul localisation
    """
    For Classification:
    ...
    if network_type == 'classification':
    ...
    elif network_type == 'localisation':
        ...
        if read_from_folders:
            ...
            images, labels  = os.listdir(input_dir), os.listdir(input_dir)
            images = [x for x in images if x.endswith(tuple(image_extensions))]
            labels = [x for x in labels if x.endswith(tuple(image_extensions))]
            images = [os.path.join(input_dir, image) for image in images]
            labels = [os.path.join(input_dir.replace('images', 'ground_truths'), label) for label in labels]
...
class CustomCallback(tf.keras.callbacks.Callback):
    def __init__(self, model, network_type, validation_data, validation_labels, major_revision, minor_revision,
                 ...
                 image_size = (608, 608), classes = [], multihead=False, multihead_classes = None, num_heads = 1):
        self.model = model
        ...
        if self.network_type == 'segmentation':
            ...
            if self.use_validation_accuracy:
                ...
                if not self.multihead_status:
                    ...
                    masks = np.zeros(predictions.shape, dtype=np.float32)

                    for i in range(len(predictions)):
                        masks[i,...] = cv2.resize(cv2.imread(self.validation_data.labels[i], -1),
                                                  predictions.shape[1:3]).reshape(*predictions.shape[1:3], 1)


                    validations_labels = masks.reshape(masks.shape[0]*masks.shape[1]*masks.shape[2],
                                                                        masks.shape[3])

                    predictions = np.array(predictions*255, dtype=np.uint8)

                    for i in range(len(predictions)):
                        th, im_gray_th_otsu = cv2.threshold(predictions[i,...].reshape(predictions.shape[1],predictions.shape[2]),
                                                            0, 1, cv2.THRESH_OTSU)
                        predictions[i,...] = im_gray_th_otsu.reshape(predictions.shape[1],predictions.shape[2],1)


                    cv2.imwrite(os.path.join('C:/Users/Asad/Desktop/epoch_results/', str(epoch+1)+'_predictions.png'),
                                np.array((predictions[0].reshape(512,512))*255, dtype=np.uint8))
                    predictions = predictions.reshape(
                        predictions.shape[0] * predictions.shape[1] * predictions.shape[2],
                        predictions.shape[3])

                    validation_accuracy = f1_score(validations_labels, predictions, average='binary')
                    ...
                    for i in range(len(predictions)):
                        th, im_gray_th_otsu = cv2.threshold(
                            predictions[i, ...].reshape(predictions.shape[1], predictions.shape[2]),
                            0, 1, cv2.THRESH_OTSU)
                        predictions[i, ...] = im_gray_th_otsu.reshape(predictions.shape[1], predictions.shape[2], 1)

                    cv2.imwrite(
                        os.path.join('C:/Users/Asad/Desktop/epoch_results/', str(epoch + 1) + '_predictions.png'),
                        np.array((predictions[0].reshape(512, 512)) * 255, dtype=np.uint8))
                    predictions = predictions.reshape(
                        predictions.shape[0] * predictions.shape[1] * predictions.shape[2],
                        predictions.shape[3])
                ...
                print('#' * 25)
                print('#### EPOCH %i' % (epoch + 1))
                print('#### F1 SCORE =%.5f' % validation_accuracy)

                self.accuracies.append(validation_accuracy)
                x = np.asarray(self.accuracies)
                if np.argsort(-x)[0] == (len(x) - self.patience - 1):
                    print('#### F1 Score no increase for %i epochs: EARLY STOPPING' % self.patience)
                    file.write('#### F1 Score no increase for %i epochs: EARLY STOPPING\n' % self.patience)
                    self.model.stop_training = True
                ...
                if (logs.get('accuracy') > 0.000) & (logs.get('accuracy') >
                                                                 np.nanmax(self.accuracies)):
                    print('#### Saving new best...')
                    file.write('#### Saving new best...\n')
                    self.model.save_weights(
...
class DataGenerator(tf.keras.utils.Sequence):
    #TODO: Change the already implemented code for localisation to correct localisation code.
    """
    ...
    def __init__(self, list_ids, labels, return_labels=True, batch_size=32, dim=(256, 256, 3), n_classes=2,
                 ...
                 multihead_classes = [], input_channels=3, normalise = False):
        ...
        self.network_type = network_type
        self.on_epoch_end()
        self.anchors = None # YOLO anchors when YOLO anchors are used
    ...
    def __data_generation(self, list_ids_temp, list_labels_temp=None):
        ...
        Generates data for a single batch containing batch_size samples as X: (n_samples, *dim) and y as (n_samples,
        max_boxes, 5) for localisation network.
        Generate data for a single batch containing batch_size samples as X: (n_samples, *dim) and y as (n_samples, *dim)
        """
        if not self.multihead:
            if self.network_type == 'classification':
                ...
                y = np.empty((len(list_ids_temp), self.dim[0], self.dim[1], self.dim[2]), dtype=np.float32)

                # Generate data
                for i, ID in enumerate(list_ids_temp):
                    if cv2.imread(ID, 0) is None:
                        print("Image not found: " + ID)
                    my_image = cv2.resize(cv2.imread(ID, 0), (self.dim[0], self.dim[1])).reshape(self.dim[0], self.dim[1], 1)
                    if self.normalise:
                    ...
                    # Store class
                    #print(list_ids_temp[i].replace('images', 'ground_truths'))
                    y[i] = cv2.resize(cv2.imread(list_ids_temp[i].replace('images', 'ground_truths'), -1), (self.dim[0], self.dim[1])).reshape(self.dim[0], self.dim[1], 1)
            ...
            elif self.network_type == 'localisation':
                X = np.empty((len(list_ids_temp), self.dim[0], self.dim[1], self.dim[2]), dtype=np.float32)
            ...
            if self.network_type == 'classification':
                ...
                for i, ID in enumerate(list_ids_temp):
                    if cv2.imread(ID, 0) is None:
                        print("Image not found: " + ID)
                    my_image = cv2.resize(cv2.imread(ID, 0), (self.dim[0], self.dim[1])).reshape(self.dim[0],
                                                                                                 self.dim[1], 1)
                    if self.normalise:
                        my_image = my_image / 255.0
                    my_shape = np.shape(my_image)
                    ...
                    # Store class
                    # print(list_ids_temp[i].replace('images', 'ground_truths'))
                    y[i] = cv2.resize(cv2.imread(list_ids_temp[i].replace('images', 'ground_truths'), -1),
                                      (self.dim[0], self.dim[1])).reshape(self.dim[0], self.dim[1], 1)
    ...
    def __getitem__(self, index):
        #TODO: Recode the code portion for localisation
        ...
        if self.network_type == 'classification':
            ...
            if self.multihead:
                X, y, y_2 = self.__data_generation(list_ids_temp=list_ids_temp)
                if self.return_labels:
                    return X, (y, y_2)
                else:
                    return X
            elif not self.multihead:
                X, y = self.__data_generation(list_ids_temp=list_ids_temp)
                if self.return_labels:
                    return X, y
                else:
                    return X
    ...
    def __random_transform(self, img, annotation):
        ...
        if self.network_type == 'classification':
            ...
            transformed_data = composition(image=img, mask=annotation)

            if self.visualise:
                temp_image = np.float32(transformed_data['image'][:, :, ::-1]).reshape(self.dim[0], self.dim[1])
                temp_mask = np.float32(transformed_data['mask'])
                if self.normalise:
                    temp_image = temp_image * 255.0

                temp_mask = temp_mask.reshape(self.dim[0], self.dim[1]) * 255.0
                horizontally_stacked = np.hstack((temp_image, temp_mask))
                cv2.imshow('Transformed Image', np.uint8(horizontally_stacked))
                cv2.waitKey()
